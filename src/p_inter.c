...
int weapontype;
 ...
 if (!source && !inflictor) // Environment kill
   {
     doom_printf("%s died due to hostile environment", target->player->name);
   } else
       if (!source->player) // Monster Kill
         {
          doom_printf("%s died to monster", target->player->name);
         } else
                ...
                switch (weapontype)
                      {
                       ...
                       case wp_betaplasma2 :
                            ...
                            break;
                       doom_printf(player_kill_msg, target->player->name);
                      }
                if (source == inflictor)
                 switch (weapontype)
                      {
                       ...
                       doom_printf(player_kill_msg, source->player->name, target->player->name);
                      }
            } else // Unknown death
                  {
                   doom_printf("death not defined");
                  }
}
