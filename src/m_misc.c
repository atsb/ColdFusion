...
#ifdef WINDOWS
    int len = snprintf(NULL, 0, "%s\\" CF_CFG, exedir);
    defaultfile = malloc(len+1);
    snprintf(defaultfile, len+1, "%s/" CF_CFG, exedir);
#else
    int len = snprintf(NULL, 0, "%s" CF_CFG, exedir);
    defaultfile = malloc(len + 1);
    snprintf(defaultfile, len + 1, "%s" CF_CFG, exedir);
